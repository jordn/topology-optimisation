...
# # Grey scale filter
# f = open('objective_function_filter1.5.csv', 'w')
# f.write('Penalty factor, Filter rad, Objective Function, Solid-void Ratio\n')

# filter_rad = 1.50
# for p_fac in range(100,200,2):
# 	tpd_filename = generatetpd(vol_frac=0.4, num_iter=250, filt_rad=filter_rad, p_fac=p_fac*0.01)
# 	optimised_t = run_through_optimisations(tpd_filename)
...
tpd_filename = generatehalfbridge(prob_name = 'half_v%.2f_f_%.1f'%(vol_frac, filter_rad), vol_frac=vol_frac, num_iter=100, \
	filt_rad=filter_rad, p_fac=p_fac, p_hold=p_hold, p_max=p_max, p_incr=p_incr, p_con=p_con)
optimised_t = run_through_optimisations(tpd_filename, print_all_images=True)
