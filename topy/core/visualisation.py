...
from pylab import axis, cm, figure, imshow, savefig, title, figaspect
...
def create_2d_imag(x, **kwargs):
    ...
    # Make the figure dimensions to closely match the shape dimensions
    # print (x.shape[1]/10, x.shape[0]/10)
    # figure(figsize=(x.shape[1]/10, x.shape[0]/10), dpi=10)
    # if kwargs.has_key('title'):
    #     title(kwargs['title'])
    # axis('off')
    # # axis('equal')
    # print 'SHAPE:', x.shape
# This ration of pixels/10 inches and a dpi of 100 makes the an order of magnitude bigger. works well for small XX x YY problems but bigger makes it silly.

    fig = figure(frameon=False, figsize=(x.shape[1]/10, x.shape[0]/10))
    imshow(-x, cmap=cm.gray, interpolation='nearest', extent=None)
    axis('off')
